...
export default {
  ...
  methods: {
    ...
    deleteBox(index) {
      ...
      Net.collectPost_Cancel(this.userInfo.uid, postId);
    },
    //清空记录
    clearAll() {
      console.log("清除所有");
      this.$createDialog({
        ...
        onConfirm: () => {
          this.listData0 = [];
          Net.collectPost_ClearAll(this.userInfo.uid);
          localStorage[LocalStorageKey["PostHasCollected"]] = JSON.stringify(
            {}
          );
          this.$createToast({
    ...
    getData0() {
      ...
      this.toast.show();
      this.page++;
      Net.getMyAudioCollect(this.userInfo.uid, this.page)
        .then(res => {
          this.toast.hide();
